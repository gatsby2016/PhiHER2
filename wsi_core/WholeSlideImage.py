...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
        ...
        def _filter_contours(contours, hierarchy, filter_params):
            """ 
                contours包含所有轮廓点，不区分foreground还是holes; foreground还是holes在hierarchy中通过层次关系体现
                Filter contours by: area.
            """
            filtered = []

            # find indices of foreground contours (parent == -1)
            hierarchy_1 = np.flatnonzero(hierarchy[:,1] == -1) #  因为cv2.RETR_CCOMP 指定仅建立两个等级的轮廓，一个上级可能有多个下级，但一个下级仅有一个上级，所以取hierarchy[1]为-1，即没有上级的轮廓编号，则为foreground。
            all_holes = []
            ...
            for cont_idx in hierarchy_1:
                ...
                holes = np.flatnonzero(hierarchy[:, 1] == cont_idx) # 寻找该foreground 轮廓点的所有下级轮廓点，即内部holes
                ...
                if a == 0: continue
                if tuple((filter_params['a_t'],)) < tuple((a,)): # 根据foreground-holes的面积确定
            ...
            for hole_ids in all_holes:
                ...
                unfilered_holes = unfilered_holes[:filter_params['max_n_holes']] # 过滤holes，每个前景轮廓内最多保留max_n_holes个
                filtered_holes = []
                
                # filter these holes 再根据面积过滤
        ...
        # cv2.RETR_CCOMP 指定仅建立两个等级的轮廓，上面一层为外边界，里面一层为内孔的边界信息；返回contours为n个轮廓点坐标集；hierarchy （1,n,4）, 为n个轮廓点的上下级层次关系；
        #4指的是用4个数表示各轮廓间的相互关系。第一个数表示同级轮廓的下一个轮廓编号，第二个数表示同级轮廓的上一个轮廓的编号，**第三个数表示该轮廓下一级轮廓的编号，第四个数表示该轮廓的上一级轮廓的编号**
        hierarchy = np.squeeze(hierarchy, axis=(0,))[:, 2:] # 只取上下级层次关系即可
    ...
    def visWSI(self, vis_level=0, color = (0,255,0), hole_color = (0,0,255), annot_color=(255,0,0), 
                    ...
                    number_contours=False, seg_display=True, annot_display=True, **kwargs):
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            if len(asset_dict) > 0:
                if init:
                    save_hdf5(save_path_hdf5, asset_dict, attr_dict, mode='w') # hd5文件保留可用patch的topleft坐标点，再理解下 TODO
                    init = False
                else:
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        x_coords, y_coords = np.meshgrid(x_range, y_range, indexing='ij') # 非常巧妙的方法，通过meshgrid取patch的topleft坐标点
        ...
        results = pool.starmap(WholeSlideImage.process_coord_candidate, iterable) # 提前判断坐标点是否在contour中，返回的都是ok的
        pool.close()
        results = np.array([result for result in results if result is not None]) # 确定的满足 contour和非holes中的patch的topleft点坐标
