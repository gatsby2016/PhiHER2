...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self,
        ...
        slide_data = self.df_prep_v2(slide_data, ignore, self.label_col) # 处理标签列，ignore特指标签列中忽略不要的case
        ...
        uncensored_df = patients_df[patients_df['死亡'] == 1] # 原处理方式censorship列0是dead，1是alive；这里和我们相反，我们这里存活是0
        ...
        slide_data = slide_data.assign(slide_id=slide_data['slide_id'])
        ...
        for i in range(len(q_bins)-1):
            for c in [0, 1]:
                ...
                label_dict.update({(i, c):key_count}) # 这里更新的0，1表示 0表示死亡，即未删失数据，1表示存活，即数据删失
                key_count+=1
        ...
        for i in slide_data.index:
            ...
            # censorship = slide_data.loc[i, '死亡']
            censorship = abs(slide_data.loc[i, '死亡'] - 1) # 原代码处理方式censorship列0是dead，1是alive；这里和我们相反，我们这里存活是0，死亡是1
        ...
        self.metadata = slide_data.columns[:15]
    ...
    @staticmethod
    def df_prep_v2(data, ignore, label_col):
        data = data[~pd.isna(data[label_col])] # 去除标签中的Nan值
...
class Generic_MIL_Survival_Dataset(Generic_WSI_Survival_Dataset):
    def __init__(self, data_dir, mode: str='omic', num_perslide=None, **kwargs):
    ...
    def __getitem__(self, idx):
        ...
        # c = torch.Tensor([self.slide_data['死亡'][idx]])
        c = torch.Tensor([abs(self.slide_data['死亡'][idx]-1)]) # 原代码处理方式censorship列0是dead，1是alive；这里和我们相反，我们这里存活是0，死亡是1；所以需要减一
            ...
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    for slide_id in slide_ids:
                        ...
                        wsi_bag = torch.load(wsi_path).type(torch.float16)
                        path_features.append(wsi_bag)
                    path_features = torch.cat(path_features, dim=0)
                    path_features = path_features[np.random.choice(len(path_features), min(self.num_perslide, len(path_features))), :]
...
class Generic_Split(Generic_MIL_Survival_Dataset):
    def __init__(self, slide_data, metadata, mode, 
        ...
        if self.genomic_features is not None:
